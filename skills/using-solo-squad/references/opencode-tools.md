...
- **Subagents:** Solo Squad registers `architect`, `tester`, `debugger`, `critic`, `sentinel`, and `reality-checker` as OpenCode subagents. Call them through the `task` tool when a workflow asks for an agent.
